...
/**
 * context
 * 此处填写绝对路径，默认值为当前目录，作为entry和loader的起点目录
 * 建议传一个值，这样能够使项目独立于当前执行目录
 */
module.exports = {
...
/**
 * 单入口
 * 当传入一个数组的时候，将创建多个主动入口，然后将多个依赖文件一起注入
 * 上面例子中，webpack将把index.js和entry.1.js一起注入，作为entry.js返回
 */
module.exports = {
  entry: {
    entry: 'index.js'
 ...
 * 多入口
 * 如上面代码中，我们可以构建多页面应用
 * 多页面应用就会涉及代码复用问题，我们可以用 CommonsChunkPlugin 提取公共代码
 * 具体使用见【webpack缓存的持久化】
 */
module.exports = {
  entry: {
...
/**
 * output
 * output对应编译后文件的出口，当资源放到CDN时，可以使用publicPath来修改对应路径，默认时可留空
 * path就对应的输出路径。filename中的[name]对应的是entry中对象的key（index，entry1）
 * [chunkhash:8]对应的是文件的8位hash值。
 * 这里的chunkFilename对应的是独立打包出来的文件，这里的name对应的是不同chunk的hash，使用好了可以实现长缓存
 * 具体使用见【webpack中样式的加载】
 */
module.exports = {
  output: {
